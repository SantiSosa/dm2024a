...
PARAM$semillas <- c(251000, 257000, 263000, 269000, 271000)
...
setwd("/Users/Fernando LA/Desktop/Data Mining") # Establezco el Working Directory
# cargo los datos
...
for (vmax_depth in c(3, 4, 5, 6, 7,8)) {
  for (vmin_split in c(200, 400, 500, 600, 700)) {
    for (cp in c(-0.1, 0, 0.1)) {
      for (vmin_bucket in c(100, 150, 200, 300, 350)) {
        ...
        tb_grid_search <- rbindlist( 
          list( tb_grid_search, 
                list( vmax_depth, vmin_split , cp,vmin_bucket, ganancia_promedio)))
  }
  }
}
